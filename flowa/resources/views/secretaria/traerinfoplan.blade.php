        ...
        <div class="bg-white rounded-lg shadow border border-gray-200">
            <div class="p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-center items-center space-y-3 sm:space-y-0 sm:space-x-4 mt-8 pt-6 border-t border-gray-200">
                    ...
                    @if($plan->estado === 'Aprobado por secretaría académica.')
                    <div class="custom-tooltip" data-tip="El programa ya fue aprobado por secretaría académica.">
                        <button type="button" class="inline-flex items-center justify-center px-5 py-2 w-70 border border-gray-300 text-sm font-medium rounded-md text-gray-400 bg-gray-100 cursor-not-allowed opacity-70">
                            APROBAR PROGRAMA
                        </button>
                    </div>
                    @else
                    <form id="approve-form" action="{{ route('secretaria.aprobarplan', ['id' => $plan->id]) }}" method="POST">
                        @csrf
                        <div class="custom-tooltip" data-tip="¿Está seguro que desea aprobar este programa?">
